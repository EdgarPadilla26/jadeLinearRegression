...
public class MultipleProfeAgent extends Agent{
	...
	private class ResolverEcuacion extends CyclicBehaviour {
		public void action() {
			...
			if (msg != null) {
                ...
                if(!ecuacion.square && ecuacion.contVar==2){

                    DataSetMultiple data = new DataSetMultiple();
                    MultipleLinearRegression solve = new MultipleLinearRegression(data.getAdv(), data.getSales(), data.getYear());
                    solve.regression();
					reply.setPerformative(ACLMessage.PROPOSE);
					reply.setContent("y = " + solve.B1 + "X + " + solve.B2 + "X_2 " + solve.B0);
