...
public class QuadraticProfeAgent extends Agent {
	...
	private class ResolverEcuacion extends CyclicBehaviour {
		public void action() {
			...
			if (msg != null) {
                ...
                if(ecuacion.square) {
                    DataSetQuadratic data = new DataSetQuadratic();
                    QuadraticRegression solve = new QuadraticRegression(data.getX(), data.getY());
                    solve.regression();
                    
					reply.setPerformative(ACLMessage.PROPOSE);
					reply.setContent("y = " + solve.B1 + "X2 + " + solve.B2 + "X + " + solve.B0);
				}
				else {
